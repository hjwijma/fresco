...
def fresco_grab(string,request="residueplus"):
    res_search = re.findall(r"(?:^|_)([A-Z](\d+)[A-Z])(?:_|-)",string)
    if request == "mutation":
    ...
    if request == "residue":
        try:
            ...
            try:
                return int(re.sub(r".*[A-Z](\d*)[A-Z].*",r"\1",string))
            except:
                return None
    if request == "disulfide":
        item = re.findall(r"((?:(?:\d)*ps_)?[A-Z]_[A-Z]\d*[A-Z]_[A-Z]_[A-Z]\d*[A-Z]_[^_]*)", string)[0]
...
if yasara.request != "CheckIfDisabled":
    ...
    if ("fresco" in yasara.workdir or "designsMD" in yasara.workdir) and not len(re.findall(r"fresco", yasara.workdir)) > 1:
        fresco_design_dir = add_sep(re.sub(r"fresco([^\\\/]*).*", "fresco\\1" + os.sep + "designsMD", yasara.workdir))
    ...
    if os.path.isfile(add_sep(fresco_design_dir) + [file for file in os.listdir(fresco_design_dir) if file.endswith("_cleaned.pdb")][0]) == True:
...
if yasara.request == "All":
    ...
    yasara.Fog(density="0%",Range="Static",dismin=str(0.5) + "%", dismax= str(1) + "%")
    ########################################################
    ...
    # Otherwise we're dealing with a disulfide bond; in this case the WTFile could be an MD snapshot
        ...
        WTList = [item for item in os.listdir(fresco_unique_dsb_dir + "Subdir_templates") if re.sub(r".*MD ((?:\d)*ps).*",r"\1", NameMutation) in item and item.endswith(".pdb")]
        if len(WTList) == 0:
            ...
            WTFile = os.path.realpath(fresco_unique_dsb_dir + "Subdir_templates") + os.sep + [item for item in os.listdir(fresco_unique_dsb_dir + "Subdir_templates") if re.sub(r".*MD ((?:\d)*ps).*",r"\1", NameMutation) in item and item.endswith(".pdb")][0]
        # For disulfide bonds the two mutant residues are simply together, separated by space, as complying with Yasara syntax
        MutatedResidue = " ".join(re.findall(r"[A-Z](\d+)[A-Z]", NameMutation))
        # Also store the subunit in which the mutation was predicted
        MutatedSU = re.findall(r", SU ([A-Z])", NameMutation)[0]
    ...
    for x in range(1, 6):
        yasara.LoadYOb(re.sub(r"_05_","_0" + str(x) + "_", MutantFile))
    ...
    while i <= len(Fresco_yob_dict):
        ...
        # ---------------------------------------------------------------
            ...
            yasara.Fog(density="0%",dismin="35%",dismax="100%")
            yasara.plugin.end()
    yasara.Console("ON")
...
# ------------------------------------------------------
    ...
    for line in excel_in:
        num = re.search(r"^(([A-Z])(\d*)([A-Z]))",line)
        ...
        if os.path.isfile(os.path.realpath(fileloc)):
            ...
            out_string = num.group(3) + "\t" + Mdexists + "\t" + re.sub(r"\s+","\t",line) + hydrophob + "\n"
            excel_out.write(out_string)

            out_string_verbose = num.group(3) + "\t" + Amino_acid_dict[num.group(2)][0] + "\t" + Amino_acid_dict[num.group(4)][0] + "\t" + Mdexists + "\t" + re.sub(r"\s+","	",line) + str(hbonds_wt) + "\t" + str(hbonds_mut) + "\t" + hbonds + "\t" + str(surfexp_wt) + "\t" + str(surfexp_mut) + "\t" + surfexp + "\t" + str(hydrophob_wt) + "\t" + str(hydrophob_mut) + "\t" + hydrophob + "\n"
    ...
    if "file_error" in locals():
        ...
        yasara.ShowWin("Custom", "Job done!",580,150,
               ...
               "Button",           285, 100, "_O_K")
    os.system(("start " if os.sep == "\\" else "open ") + fresco_design_dir + os.sep + Fresco_wildtype + "MutantsForInspections_XL.tab")
    yasara.Console("ON")
