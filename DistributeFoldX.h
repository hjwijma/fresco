...
*                                                                                                                                                          *
*                                                                                                                                                          *
*  Author: Hein Wijma, University of Groningen, October 2010, h.j.wijma@rug.nl                                                                             *
*                                                                                                                                                          *
*                                                                                                                                                          *
***********************************************************************************************************************************************************/


/* by setting this to 1, much more information is written to the LOG, and it is easier to see what is going (wr)on(ng) */
#define DEBUG 0
...
/* this produces a list of all the mutations in the case of saturation scanning mutagenesis */
struct MutationProtein * SaturationScanning(struct SequenceProtein *);
...
/* This makes the subdirectories to do the FoldX calculations in */
void MakeSubdirectories( int NumberOfSubdirectories);

/* these are identical to standard functions (itoa/reverse) that are sometimes absent from a library  */
...
struct SequenceProtein * LoadYasaraTable(char *NameTableFile)
{
  ...
  /* put the data from the file into the SequeceProtein Structure ,assume all the lines are useful to read unless at says END */
...
/* This function removes all mutations with a value above the cuttoff value.  */
...
void short_info()
{
   ...
   printf("WARNING: The files .. and a suitable version of .. should be present in the current directory\n\n");
}
...
void example_use()
{
   ...
   printf("\tDistributeFoldX Phase1 MyPrecious 2 A B MyPreciousSelection.tab 100 /home/wijma/Fold_X/FoldX.linux64 \n\n");
   printf("Should prepare mutations of MyPrecious.pdb in the 2 subunits A and B of the residues in the File MyPreciousSelection.tab and\n");
   printf("distribute them over directories with each a 100 different mutations\n\n");
   
   printf("\tDistributeFoldX Phase2 MyPrecious 2 A B MyPreciousSelection.tab 100 10 \n\n");
   printf("Should COLLECT the mutations of MyPrecious.pdb in the 2 subunits A and B of the residues in the File MyPreciousSelection.tab and\n");
   printf("AND make FOUR lists: a completelist, a separate list of the mutations that are less than 10 kJ mol -1 harmful, a list with the best \n mutation for every position, (and a similar list with less than 10 kJ mol-1 damage).\n\n");
}
...
/*identical to standard itoa function */
void titoa(int n, char s[])
{
  int i, sign;
