...
CurrentSetting = 'MDforThermostabilityScreening'
...
if (CurrentSetting == 'MDforThermostabilityScreening')
  WarmUpRegime           = 'N'
...
if !(UnMinimizedStructureExists)
  ...
  #CELL AUTO, extension = 7.5, Shape=Dodecahedron
  ...
  SIM OFF
  # This removes new water molecules near the substrate, turns out to give more accurate result
  DELRES OBJ 3 Res HOH with distance < 2.8 from Res Sub
  
  # if we are screening for thermostability we are interested in surface bound waters, to find those they need to be in the same object as the protein
  if (CurrentSetting == 'MDforThermostabilityScreening')
    JOINOBJ 3,1
...
for CSN = 01 to (MaximumSeeds)
  ...
  for c = 1 to count Label_
    (Label_(c))s(CSN)  = (IntermediateList(14 + (c*4)))
    (Label_(c))SD(CSN) = (IntermediateList(14 + (c*4)+1))
    (Label_(c))Pass(CSN) = (IntermediateList(14 + (c*4)+2))    
  PRINT round (CSN) of CSN = 01 to (MaximumSeeds)
...
if MakeSnapshotPdbs == 'Yes'
  for i = 1 to count SnapShotPdbsTimeps 
    ...
    SAVEPDB OBJ 1, (MacroTarget)_(SnapShotPdbsTimeps(i))ps
...
if UseRMSFInPdbYOBFile== 'TRUE'
  ...
  # Also get rid of all the mobile water atoms and ions, 5 is already quite flexible, if still too high
  DELRES HOH CIP CIM bfactor > 5
  ...
  for i = 00000 to ((count TimeSnapShot) - 1)
    ...
    for CSN = 01 to (MaximumSeeds)
      ...
      if (NextSnaphotExists)
        ...
        SIM Off
        ADDOBJ (OriginalStructure)
        SUPATOM OBJ 1 protein atom element !H, OBJ (OriginalStructure) protein atom element !H
        ADDPOSATOM OBJ 1
        REMOVEOBJ (OriginalStructure)
      else 
        GreenFlag = 0
